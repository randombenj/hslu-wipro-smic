...
def step_prepare_api(context):
    await_data_post("http://localhost:8080/db/setDefaults", None)


@when("assigning a label")
def step_assign_label(context):
    await_data_post("http://localhost:8080/meters/2/labels",
...
def step_check_label_assigned_correctly(context):
    response = await_data_request("http://localhost:8080/labels/assignments")
